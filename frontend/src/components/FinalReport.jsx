...
<body>
  <div class="header">
    <div class="brand">🔥 Agneyaa · ${t('report_title')}</div>
    <h1>${bc.full_name || t('form_full_name')} — ${bc.business_idea || t('dashboard_idea')}</h1>
    <div class="meta">
      ${t('form_category')}: ${cat} &nbsp;·&nbsp;
      ${t('dashboard_location')}: ${[bc.village, bc.block, bc.district].filter(Boolean).join(', ') || '—'} &nbsp;·&nbsp;
      Date: ${dateStr} &nbsp;·&nbsp;
  ...
  <div class="score-box">
    ...
    <div class="score-info">
      ...
      <div class="verdict-desc">
        ${scoreNum >= 7.5
          ? `High potential. Recommended to proceed.`
          : scoreNum >= 5.5
          ? `Viable with conditions. Start smaller scale.`
          : `More market research recommended before proceeding.`
        }
      </div>
      <div class="factors">
        ${factors.map(f => `
  ...
  <div class="card">
    ...
    <table>
      <tbody>
        ${[
          [t('form_full_name'), bc.full_name || '—'],
          [t('form_business_idea'), bc.business_idea || '—'],
          [t('form_category'), cat],
          [t('dashboard_location'), [bc.village, bc.block, bc.district].filter(Boolean).join(', ') || '—'],
          [t('mool_own_capital'), fmtINR(Number(bc.own_capital)) || '—'],
          [t('form_caste'), bc.caste_category || '—'],
          [t('form_land'), bc.land_owned || '—'],
          [t('form_target_customers'), bc.target_customers || '—'],
  ...
  <div class="card">
    <div class="card-title">💰 ${t('report_financial')}</div>
    <div class="fin-grid">
      ${[
        [t('mool_project_cost'), fmtINR(md.projectCost || Number(bc.own_capital) * 10)],
        [t('mool_own_capital'), fmtINR(Number(bc.own_capital))],
        [t('mool_net_loan'), fmtINR(md.netLoan || Number(bc.own_capital) * 8)],
        [t('mool_emi'), fmtINR(md.emi || 0)],
        [t('mool_subsidy_amt'), fmtINR(md.subsidy || 0)],
        [t('mool_working_capital'), fmtINR(md.workingCap || 0)],
      ].map(([l, v]) => `<div class="fin-item"><div class="fin-label">${l}</div><div class="fin-value">${v}</div></div>`).join('')}
    </div>
    ${md.scheme ? `<p style="margin-top:10px;font-size:12px;color:#555"><strong style="color:#1a5c2a">${scheme.name}</strong> · ${scheme.rate}% p.a. · ${t('mool_subsidy_amt')}: ${scheme.subsidy} · ${t('mool_moratorium')}: ${scheme.moratorium || 0} ${t('mool_months')}</p>` : ''}
  </div>

  <div class="card">
    <div class="card-title">📊 Market Analysis</div>
    <ul style="padding-left:16px;margin:0;font-size:12px;line-height:1.7">
      <li><strong>${t('dashboard_location')}:</strong> ${bc.village || 'Your location'}, ${bc.district} — verified via India Post API.</li>
      ...
      <li><strong>${t('form_target_customers')}:</strong> ${bc.target_customers || 'Local households and community members'}.</li>
    </ul>
  </div>

  <div class="card">
    <div class="card-title">${t('report_risk_matrix')}</div>
    <table>
      <thead><tr><th>${t('report_risk')}</th><th>${t('report_level')}</th><th>${t('report_mitigation')}</th></tr></thead>
      <tbody>
        ${risks.map(r => `
          <tr>
            <td>${r.risk}</td>
            <td class="risk-${r.level.toLowerCase()}">${t('report_' + r.level.toLowerCase()) || r.level}</td>
            <td>${r.mitigation}</td>
          </tr>
        `).join('')}
      </tbody>
  ...
  <div class="card">
    <div class="card-title">${t('report_documents')}</div>
  ...
  <div class="card">
    <div class="card-title">${t('report_action_plan')}</div>
  ...
  return (
    <div className={styles.wrap} ref={reportRef}>
      ...
      <div className={styles.grid}>
        ...
        {/* Financial Overview */}
        <div className={styles.card}>
          <div className={styles.cardTitle}>{t('report_financial')}</div>
        ...
        {/* Risk Assessment */}
        <div className={styles.card}>
          <div className={styles.cardTitle}>{t('report_risk_matrix')}</div>
          <div style={{ overflowX: 'auto' }}>
            <table className={styles.riskTable}>
              <thead>
                <tr><th>{t('report_risk')}</th><th>{t('report_level')}</th><th>{t('report_mitigation')}</th></tr>
              </thead>
              <tbody>
                {risks.map((r, i) => (
                  <tr key={i}>
                    <td>{r.risk}</td>
                    <td className={r.level === 'High' ? styles.riskHigh : r.level === 'Medium' ? styles.riskMedium : styles.riskLow}>
                      {r.level === 'High' ? t('report_high') : r.level === 'Medium' ? t('report_medium') : t('report_low')}
                    </td>
                    <td>{r.mitigation}</td>
                  </tr>
                ))}
        ...
        {/* Document Checklist */}
        <div className={styles.card}>
          <div className={styles.cardTitle}>{t('report_documents')}</div>
        ...
        {/* Action Plan */}
        <div className={styles.card}>
          <div className={styles.cardTitle}>{t('report_action_plan')}</div>
